# Cross-servicer dataflow / lineage — 待分析占位 / Pending analysis placeholder (zh)

> **状态**：⏳ pending-deferred（v9.1 placeholder）
> **Purpose**：占位文档。Cross-servicer dataflow / lineage 的 Validation Report 生成逻辑**尚未**分析。
> 本文件保证 mkdocs 站点导航和 servicers-registry 中存在 Cross-servicer dataflow / lineage 的位置，
> 避免遗忘；待分析完成后，本文件会被 toc/rawdata/dataflow/sheets/fields/rules
> 章节集合替换。
>
> **Intended audience**：未来接手 Cross-servicer dataflow / lineage 分析的 session agent + 用户。
>
> **Revision history**
>
> | Date | Author | Change |
...
1. Cross-servicer dataflow / lineage 共有多少个 @task validator？分别叫什么名字？
...
5. Cross-servicer dataflow / lineage 在 `port.portmonth` 中的 `servicer` 字段值是什么？
6. 是否有 Cross-servicer dataflow / lineage-specific schema（如 `dataflow.*`）作为 unified 层？
7. 已知缺陷或边界情况？
...
1. 创建 `docs/dataflow/toc.{zh,en}.md`、`rawdata.{zh,en}.md`、`dataflow.{zh,en}.md`、`sheets.{zh,en}.md`、`fields.{zh,en}.md`、`rules.{zh,en}.md`，参考 MRC 章节的模板结构。
2. 更新 `mkdocs.yml` 导航：把 Cross-servicer dataflow / lineage 从 "Pending servicers" 移到正式章节区。
3. 在 `tools/stage_doc_checks.py` 的 PAIRS 列表中添加 Cross-servicer dataflow / lineage 的所有新文档对。
4. 更新 `docs/_status/servicers-registry.{zh,en}.md` + `plan.md` 状态矩阵：⏳ → ✅。
5. 在 `test_reports/` 写测试报告。
6. SQL todo（`stage1-dataflow` 或新拆分的 `stage1-dataflow-*`）状态从 `pending-deferred` → `done`。
