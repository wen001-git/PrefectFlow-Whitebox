...
> **另见 / See also — G2a Redshift 依赖**
>
> 执行 G2a 快照导出（`freeze_snapshot.py export`）需要 Redshift VPN 访问权限与 Vault
> token。在运行导出之前，请阅读一站式前提检查清单：
...
>
> *Running G2a snapshot export requires Redshift VPN + Vault token. Prerequisite checklist: see links above.*
