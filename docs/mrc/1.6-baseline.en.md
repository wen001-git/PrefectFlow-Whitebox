...
> | 1.5 | Validation Rules / 验证规则 | `1.5-rules.en.md` | Rule catalog |
...
1.6 Baseline XLSX Behavior (1.6-baseline.en.md) vs. the preceding 5 chapters:

- 1.3 Sheet Rendering Layer (1.3-sheets.en.md) describes the **rendering mechanism** (what the code does); this chapter describes the **rendering output** (what the final XLSX looks like).
- 1.4 Field Definitions (1.4-fields.en.md) describes **column lineage** (where each column comes from); this chapter describes the **physical XLSX appearance of each column** (number_format, width, header style).
- 1.5 Validation Rules (1.5-rules.en.md) describes **decision rules** (when something counts as an exception); this chapter describes the **visual encoding of an exception in the XLSX** (exact RGB of pink fill / orange font).

This chapter does **not**:

- Restate the rendering pipeline — see 1.3 Sheet Rendering Layer (1.3-sheets.en.md) § 3.
- Define business rules — see 1.5 Validation Rules (1.5-rules.en.md).
- Describe upstream SQL — see 1.2 Dataflow Layer (1.2-dataflow.en.md).
...
- `[FROM-CODE]`: **fully determined by source code**, reverse-engineered from `gen_remit_validation_report.py` (see 1.3 Sheet Rendering Layer (1.3-sheets.en.md) § 4). Stage 2 implementers may treat this chapter's description as the contract directly.
...
- Upstream Parquet snapshot schema — see 1.1 Raw Data Layer (1.1-rawdata.en.md).
...
Source: `gen_remit_validation_report.py:1327-1356`; see 1.3 Sheet Rendering Layer (1.3-sheets.en.md) § 4.4.
...
Exact highlight positions: 1.3 Sheet Rendering Layer (1.3-sheets.en.md) §§ 5–9 column catalogs.
...
Applies to: every column with `data_type == money` (see 1.3 §§ 5–9 / 1.4 Field Definitions (1.4-fields.en.md) data_type column).
...
No MRC column uses this type; listed here for completeness only. If mis-used it would apply `0.00%` (`percent_format`). `intrate_*` columns are **deliberately** declared `float` rather than `percentage` — see 1.3 § 4.2 + 1.4 Field Definitions (1.4-fields.en.md) § 5 gap 1.
...
Rows where `mask == True` get the diff style applied. Detailed rules: 1.5 Validation Rules (1.5-rules.en.md) § 3.1 and § 3.2.
...
- `pandi_diff_remitvsdaily` (column 31 of `MRC_General_Check`) — actual-P&I delta, **deliberately** excluded from the highlight list (noise floor too high); see 1.3 § 6.1 gap 1 and 1.5 Validation Rules (1.5-rules.en.md).
...
Further rendering cross-references: 1.3 Sheet Rendering Layer (1.3-sheets.en.md) § 4; rule cross-references: 1.5 Validation Rules (1.5-rules.en.md) § 3.
