...
> **Purpose**: Reverse-engineer the per-sheet rendering layer of the MRC Validation Report — for each of the 5 `MRC_*` XLSX sheets, document the exact column ordering, data-type / rounding contract, highlight columns and visual style, and the openpyxl machinery that turns the validator DataFrames (from 1.2 Dataflow Layer (1.2-dataflow.en.md)) into the final workbook cells.
>
> **Audience**: Stage 1 reviewers; the engineer who will write Stage 2's MRC sheet renderer; future Copilot CLI agents resuming 1.3 Sheet Rendering Layer (1.3-sheets.en.md) / 1.4 / 1.6 work.
>
> **Revision history**
>
> | Date | Author | Change |
...
- Threshold semantics (always `0` for MRC) are noted; the underlying
  comparison logic moves to 1.5 Validation Rules (1.5-rules.en.md).

## 3. Overall rendering pipeline

```mermaid
flowchart LR
    map["VALIDATION_TABLE_MAP\n(5 MRC keys → DataFrames)\nbuilt in 1.2 Dataflow Layer (1.2-dataflow.en.md)"] --> get["get_cfg(setting)\nbuilds SheetConfig per sheet"]
...
- For `data_type == "percentage"` columns: cells get `percent_format` =
  ...
  percentage; 1.4 Field Definitions (1.4-fields.en.md) will revisit this design choice.
...
Source: `gen_remit_validation_report.py:1180-1196, 1327`; 1.2 Dataflow Layer (1.2-dataflow.en.md) § 6.1.
...
Source: `gen_remit_validation_report.py:1199-1236, 1328-1340`; 1.2 Dataflow Layer (1.2-dataflow.en.md) § 5.
...
Source: `gen_remit_validation_report.py:1239-1268, 1341-1350`; 1.2 Dataflow Layer (1.2-dataflow.en.md) § 4.
...
Source: `gen_remit_validation_report.py:1271-1281, 1351-1355`; 1.2 Dataflow Layer (1.2-dataflow.en.md) § 6.2.
...
- **Dependencies / assumptions**: `servicefee_diff` is `NULL` whenever `port.portmonth` lacks the loan (1.2 Dataflow Layer (1.2-dataflow.en.md) § 6.2) — `NULL` does not highlight (silent miss); `fctrdt` is duplicated information vs `asofdate` (former is the SQL filter value, latter is `mrc_db.remit_date`) — documented as gap 4.
...
Source: `gen_remit_validation_report.py:1284-1293, 1356`; 1.2 Dataflow Layer (1.2-dataflow.en.md) § 6.3.
...
- **Key transformations**: `amt_MoM = amt / amt_1m - 1` retains `±inf` / `NaN` from the validator (1.2 Dataflow Layer (1.2-dataflow.en.md) § 6.3); `data_type_format` does **not** format `float`-typed cells, so `inf` / `NaN` flow through as openpyxl writes them (Excel's representation of `inf` is `1.7976931348623157e+308`-style — must be verified during 1.6 baseline capture, recorded as gap 5).
