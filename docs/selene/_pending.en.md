# Selene — Pending analysis placeholder (en)

> **Status**: ⏳ pending-deferred (v9.1 placeholder).
> **Purpose**: placeholder doc. Selene's Validation Report generation logic
> has **not** yet been analyzed. This file guarantees the mkdocs nav and the
> servicers-registry both reserve a slot for Selene so it is not forgotten;
> when analysis happens, this file will be replaced by the toc / rawdata /
> dataflow / sheets / fields / rules chapter set.
>
> **Intended audience**: the future session agent picking up Selene +
> the user.
>
> **Revision history**
>
...
1. How many @task validators does Selene have? What are their names?
...
5. What is the `servicer` field value in `port.portmonth` for Selene?
6. Is there a Selene-specific schema (e.g. `selene.*`) serving as the unified layer?
...
1. Create `docs/selene/toc.{zh,en}.md`, `rawdata.{zh,en}.md`, `dataflow.{zh,en}.md`, `sheets.{zh,en}.md`, `fields.{zh,en}.md`, `rules.{zh,en}.md` mirroring the MRC chapter template.
2. Update `mkdocs.yml` nav: move Selene from "Pending servicers" to the active chapter section.
...
6. Move the SQL todo (`stage1-selene` or the further-split `stage1-selene-*`) from `pending-deferred` → `done`.
