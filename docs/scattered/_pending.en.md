# Scattered (cross-servicer validators) — Pending analysis placeholder (en)

> **Status**: ⏳ pending-deferred (v9.1 placeholder).
> **Purpose**: placeholder doc. Scattered (cross-servicer validators)'s Validation Report generation logic
> has **not** yet been analyzed. This file guarantees the mkdocs nav and the
> servicers-registry both reserve a slot for Scattered (cross-servicer validators) so it is not forgotten;
> when analysis happens, this file will be replaced by the toc / rawdata /
> dataflow / sheets / fields / rules chapter set.
>
> **Intended audience**: the future session agent picking up Scattered (cross-servicer validators) +
> the user.
>
> **Revision history**
>
...
1. How many @task validators does Scattered (cross-servicer validators) have? What are their names?
...
5. What is the `servicer` field value in `port.portmonth` for Scattered (cross-servicer validators)?
6. Is there a Scattered (cross-servicer validators)-specific schema (e.g. `scattered.*`) serving as the unified layer?
...
1. Create `docs/scattered/toc.{zh,en}.md`, `rawdata.{zh,en}.md`, `dataflow.{zh,en}.md`, `sheets.{zh,en}.md`, `fields.{zh,en}.md`, `rules.{zh,en}.md` mirroring the MRC chapter template.
2. Update `mkdocs.yml` nav: move Scattered (cross-servicer validators) from "Pending servicers" to the active chapter section.
...
6. Move the SQL todo (`stage1-scattered` or the further-split `stage1-scattered-*`) from `pending-deferred` → `done`.
...
Inventory of validators not yet enumerated. Per v8 plan, ~8 cross-servicer validators. Analyze validator names, what they check, and which sheets they contribute to.
